...
def convert_to_square(image, padding_color: tuple):

    img_height, img_width = image.shape[:2]
    target_size = max(img_width, img_height)

    append_dist = int(abs(img_height - img_width) / 2)

    if image.ndim == 3:
        new_img = np.ones((target_size, target_size, 3), dtype=np.uint8)
        new_img[:, :, 0] = new_img[:, :, 0] * padding_color[0]
        new_img[:, :, 1] = new_img[:, :, 1] * padding_color[1]
        new_img[:, :, 2] = new_img[:, :, 2] * padding_color[2]
    else:
        new_img = np.ones((target_size, target_size), dtype=np.uint8) * padding_color[0]

    append_axis = 0
    if img_height < img_width:
        new_img[append_dist:append_dist + img_height, :] = image
        append_axis = 1
    else:
        new_img[:, append_dist:append_dist + img_width] = image

    return new_img, append_dist, append_axis